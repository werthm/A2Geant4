...
G4VPhysicalVolume* A2SolidTargetGeneric::Construct(G4LogicalVolume* motherLogic, G4double z0)
{
    ...
    switch (fType)
    {
        ...
        case kOct_18:
        case kJan_19:
            G4cout << "A2SolidTargetGeneric::Construct(): Constructing solid target for October/November/January 2018 experiments" << G4endl;
    ...
    // set material of 'air' tube : either air or vacuum
    G4Material* airTube_mat = fNistManager->FindOrBuildMaterial("G4_AIR");
    if (fType == kOct_18 || fType == kJan_19)
    {
        airTube_mat = new G4Material("Solid_Target_Vacuum", 1.188e-5*kg/m3, 1, kStateGas, 293.15*kelvin, 1e-5*bar);
        airTube_mat->AddMaterial(fNistManager->FindOrBuildMaterial("G4_AIR"), 1.);
    }

    // construct the air tube
    G4Tubs* airTube = new G4Tubs("TRGT", 0, airTube_radius, airTube_length / 2. + 70*um, 0*deg, 360*deg);
    fMyLogic = new G4LogicalVolume(airTube, airTube_mat, "TRGT");
    ...
    if (fType == kOct_18 || fType == kJan_19)
    {
        ...
        // forward hole
        G4Tubs* subtr_hole_forward = new G4Tubs("SUBTR_HOLE_FORWARD", 0, fRadius+0.2*mm, 12.5*mm/2., 0*deg, 360*deg);
        ...
        // shifted holder for Pb-208 target
        if (fMaterial->GetName() == "A2_NS_Pb208")
            v = new G4PVPlacement(0, G4ThreeVector(0, 0, z0 - airTube_z0 + 10.5*mm), t_holder_log, "TRGT_HOLDER", fMyLogic, false, 0);
        else
